...
module.exports = function(objectSchemaDatabase,log){
	...
	var href = function(request,resourcePath){
		var host = request.info.host;
		if(!host){
			var href = request.server.info.uri;
			var startIndex = href.indexOf('//');
			host = href.substring(startIndex + 2,href.indexOf(':',startIndex));
		}
		
		return request.server.info.protocol + '://' + host + ':' + request.server.info.port + resourcePath;
	};
				...
				var addLinks = function(){
					if(params.links){
						...
						searchResults.links = [
							{
								rel: 'self',
								href : href(request,path() + linkQueryString),
